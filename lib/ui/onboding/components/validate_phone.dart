...
class ValidatePhoneForm extends StatefulWidget {
  ...
  @override
  State<ValidatePhoneForm> createState() => _ValidatePhoneFormState();
}

class _ValidatePhoneFormState extends State<ValidatePhoneForm> {
  ...
  void validatePhoneNumber(BuildContext context) {
    ...
    Future.delayed(const Duration(seconds: 1), () async {
      if (_formKey.currentState!.validate()) {
        final phoneNumber = int.tryParse(inputPhone.text);
        if (phoneNumber == null) {
          setState(() {
            isShowLoading = false;
          });
        ...
        final resp = await Api().verifiedPhone(phoneNumber.toString());

        print(resp);
        if (resp?.data?['statusCode'] == 401 || resp?.data?['statusCode'] == 400 || resp?.data?['statusCode'] == 500) {
          error.fire();
